...
export default function CasinoCategory() {
  ...
  const getCurrentActiveCategory = useCallback(() => {
    ...
    console.log('Active category index:', categoryIndex !== -1 ? categoryIndex : 0);
    return categoryIndex !== -1 ? categoryIndex : 0; // По умолчанию "all"
  }, [ALL]);
  ...
  const handleCategoryClick = (row: any, index: number) => {
    ...
    if (row.url) router.push(`/${currentLang.value}${row.url}`);
  };
      ...
      <Box sx={{ px: 0.1 }}>
        ...
        >
          {ALL.map((item, index) => {
            ...
            console.log(
              `Rendering ${item.label} (index: ${index}): isActive = ${isActive}, activeCategory = ${activeCategory}`
            );
            return (
              <Typography
                key={index}
